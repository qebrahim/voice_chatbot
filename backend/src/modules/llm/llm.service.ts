...
export class LlmService {
  ...
  async generateResponse(userMessage: string, conversation: any = null): Promise<string> {
    const messages = [
      {
        role: 'system',
    ...
    try {
      ...
      const historyContents: Array<{ role: string; parts: { text: string }[] }> = [];
      ...
      const aiResponse: string = candidate?.content?.parts?.[0]?.text?.trim?.() || '';
